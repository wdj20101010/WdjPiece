...
public class Wdj1 {
	...
	//流程处理
	//处理enter{},处理 result{},处理runnow{},处理runlater{}
		        ...
		        //runnow{}语句执行，runlater{}语句执行
		        	...
		        	for(int j=0;j<str2.length();j++)
			        	if((ch=str2.charAt(j))!=' ')str3=str3+ch;
			        		...
			        		if(DP.isHave(str3)&&DP.getProce(str3)!=null)runlist.add(str3);
			        	}
		        	if(runlist.size()>0&&str1.indexOf("runnow{")==0)DP.RunAddHead(runlist);
		        	else if(runlist.size()>0&&str1.indexOf("runlater{")==0)DP.RunAddTail(runlist);
		        }
		        str1="";
			}
		    //处理entfrom(){},处理 resulto(){},处理if(){},处理  while(){}
