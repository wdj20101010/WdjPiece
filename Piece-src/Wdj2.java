...
public class Wdj2 {
	...
	Wdj2(DataPool dp,String thisPiece){
	...
	public void equateOperate(String str1,String str2)throws Exception{
		...
		while(true){
		    ...
		    //是函数
			    ...
			    if(括号==0){new Wdj3(DP,ThisPiece).functOperate(strx,stry.substring(0, i-1));str2=strx;}
			    if(i<stry.length()&&stry.charAt(i)=='.')str2=strx+stry.substring(i);
		    }
		    else break;
		}
		//是 null Null NULL
		if(str2.equals("null")||str2.equals("Null")||str2.equals("NULL"))DP.reMove(str1);
		//是布尔
		...
		for(int i=0;i<算式.size();i++){
			...
			while(DP.isHave(str1)){
				...
				if(i+2<算式.size()&&算式.get(i+1).getstr().equals("(")){
					...
					new Wdj3(DP,ThisPiece).functOperate(str1,str2);
				}
