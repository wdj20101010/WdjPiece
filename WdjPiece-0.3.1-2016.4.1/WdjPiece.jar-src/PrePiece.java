...
public class PrePiece {
	...
	private void mainOperate(String str)throws Exception{
		...
		for(int i=0;i<str.length();i++)
		    ...
		    //处理一般代码
			else if(str1.indexOf("include{")!=0){str2=str2+str1+" ";str1="";}
		    //处理include{}语句
		        ...
		        if(大括号!=0)throw new Exception("include{}语句错误");
		        //include{}语句执行
		        strx=strx+' ';String stry="",strz="";int chx;
		        for(int j=0;j<strx.length();j++)
		        	if((ch=strx.charAt(j))!=' '&&ch!=',')stry=stry+ch;
		        	else if(stry.length()>0){
		        		int k=0;if(k<源码名.size()&&!stry.equals(源码名.get(k)))k++;
		        		if(k==源码名.size()){
			        		FileReader filein=new FileReader(stry);
