...
public class Wdj3 {
	...
	public void functOperate(String name,String parame)throws Exception{
		String str[]=DP.getfunct(name),str1="",str2="",str3;
		...
		for(int i=0,j=0;i<str[0].length()&&j<parame.length();i++,j++){
			...
			//标记参数
			str3="";
			if(str1.indexOf('.')!=-1)str3=str1.substring(0, str1.indexOf('.'));
			if(str3.length()>0&&!DP.ishave(str3))TempPiece.add(str1);
			else if(str3.length()==0&&!DP.ishave(str1))TempPiece.add(str1);
			//参数赋值
